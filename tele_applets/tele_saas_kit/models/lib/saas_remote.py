...
class tele_remote_container:
    def __init__(self, db="dummy", tele_image="tele:1.0", tele_config = None ,host_server = None, db_server = None, version = "1.0"):
...
#        used_ports = [8888] #8888 to be used for DB templates 
        for each in containers:
            ...
            if port_info and port_info.get('9000/tcp',None):
#                used_ports.append(port_info['9000/tcp'][0]['HostPort'])
                self.ports_in_use.add(port_info['9000/tcp'][0]['HostPort'])
                if port_info.get('9001/tcp',None):
                    self.ports_in_use.add(port_info['9001/tcp'][0]['HostPort'])
#
#        return used_ports
    ...
    def mkdir_mnt_extra_applets(self, folder):
        try:
            ...
            if self.execute_on_remote_shell(ssh_obj,cmd):
                self.response['common-applets'] = path
                return path
        except Exception as e:
    ...
    def run_tele(self,name, db):
        ...
        try:
            ...
            self.dclient.containers.run(image=self.tele_image,name=name,detach=True,volumes={extra_path:{'bind':self.data_dir,"mode":"rw"}, path: {'bind': "/etc/tele/", 'mode': 'rw'},self.common_applets:{'bind': "/mnt/common-applets", 'mode': 'rw'}},ports={9000:port, 9001 : lport },tty=True,restart_policy={"Name":"unless-stopped"}) #Start the container
...
class nginx_vhost:
    ...
    def __init__(self,vhostTemplate="vhosttemplate.txt",sitesEnable = '/var/lib/tele/tele-data/docker_vhosts/',sitesAvailable = '/etc/nginx/sites-available/'):    
...
def create_db_template(db_template=None,modules=None, config_path=None,host_server = None, db_server = None, version = "1.0"):
    ...
    sitesEnable = TeleObject.tele_config+"/docker_vhosts/"
    host_domain = "db1_templates."+host_server['server_domain']
    ...
    if not TeleObject.is_container_available(TeleObject.tele_template):
        try:
            ...
            TeleObject.dclient.containers.run(image = TeleObject.tele_image, name = TeleObject.tele_template, detach = True, volumes = {extra_path:{'bind':TeleObject.data_dir,"mode":"rw"}, path: {'bind': "/etc/tele/", 'mode': 'rw'}, TeleObject.common_applets:{'bind': "/mnt/common-applets", 'mode': 'rw'}}, ports = {9000:TeleObject.template_tele_port , 9001:TeleObject.template_tele_lport }, tty = True,restart_policy={"Name":"unless-stopped"}) #Start the container
            _logger.info("Let's give Tele 2s")
            time.sleep(2)
