...
class ComponentBuilder(models.AbstractModel):
    ...
    Every time an Tele registry is built, the know components are cleared and
    rebuilt as well.  The Component classes are built using the same mechanism
    than Tele's Models: a final class is created, taking every Components with
    a ``_name`` and applying Components with an ``_inherits`` upon them.

    The final Component classes are registered in global registry.

    This class is an Tele model, allowing us to hook the build of the
    components at the end of the Tele's registry loading, using
    ...
    def _register_hook(self):
        # This method is called by Tele when the registry is built,
    ...
    def build_registry(self, components_registry, states=None, exclude_applets=None):
        if not states:
            states = ("installed", "to upgrade")
        # lookup all the installed (or about to be) applets and generate
        # the graph, so we can load the components following the order
        # of the applets' dependencies
        ...
        if exclude_applets:
            ...
            params.append(tuple(exclude_applets))
    ...
    def load_components(self, module, components_registry=None):
        """Build every component known by MetaComponent for an tele module

        The final component (composed by all the Component classes in this
        module) will be pushed into the registry.

        :param module: the name of the applet for which we want to load
                       the components
