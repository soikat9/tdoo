...
from tele.tools import LastOrderedSet, OrderedSet
...
# this is duplicated from tele.models.MetaModel._get_applet_name() which we
# unfortunately can't use because it's an instance method and should have been
# a @staticmethod
def _get_applet_name(full_name):
    # The (Tele) module name can be in the ``tele.applets`` namespace
    # or not. For instance, module ``sale`` can be imported as
    # ``tele.applets.sale`` (the right way) or ``sale`` (for backward
    # compatibility).
    module_parts = full_name.split(".")
    if len(module_parts) > 2 and module_parts[:2] == ["tele", "applets"]:
        applet_name = full_name.split(".")[2]
    else:
        applet_name = full_name.split(".")[0]
    return applet_name
...
class ComponentRegistry:
    ...
    the components, applets loaded first have their components found first.
    ...
    def lookup(self, collection_name=None, usage=None, model_name=None):
        ...
        # keep the order so applets loaded first have components used first
        candidates = (
            component
...
# it will be cleared and updated when the tele's registry is rebuilt
...
class WorkContext:
    ...
    .. attribute:: model_name
        ...
        as a `model` attribute to use directly with the Tele model from
        the components

    .. attribute:: collection

        The collection we are working with. The collection is an Tele
        Model that inherit from 'collection.base'. The collection attribute
        can be a record or an "empty" model.

    .. attribute:: model

        Tele Model for ``model_name`` with the same Tele
        :class:`~tele.api.Environment` than the ``collection`` attribute.
        ...
        if components_registry is not None:
            ...
            try:
                ...
                _logger.error(
                    ...
                    "Probably because the Tele registry has not been built "
                    "yet.",
                    dbname,
                )
                raise
    ...
    def component_by_name(self, name, model_name=None):
        ...
        A :exc:`tele.applets.component.exception.NoComponentError` is raised
        if:
    ...
    def component(self, usage=None, model_name=None, **kw):
        ...
        A :exc:`tele.applets.component.exception.SeveralComponentError` is
...
class MetaComponent(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        # "components of the applet" list. If not, when we use the
        ...
        # second tests uses the "load_components" to load all the applets of the
        # module: it will load the component of the previous test.
        if "tests" in cls.__module__.split("."):
            return

        if not hasattr(cls, "_module"):
            cls._module = _get_applet_name(cls.__module__)
...
class AbstractComponent(metaclass=MetaComponent):
    ...
    Inheritance mechanism
        The inheritance mechanism is like the Tele's one for Models.  Each
        ...
        familiar if you already know Tele.  The first uses ``_inherit`` with
    ...
    Registration and lookups
        ...
        _apply_on
            List of names or name of the Tele model(s) for which the component
    ...
    * It is advised to namespace the names of the components (e.g.
      ``magento.xxx``) to prevent conflicts between applets.
    ...
    @property
    def env(self):
        """Current Tele environment, the one of the collection record"""
    ...
    def _build_component(cls, registry):
        ...
        This method is called at the end of the Tele's registry build.  The
        ...
        :meth:`tele.applets.connector.components.mapper.Mapper._complete_component_build`

        The following code is roughly the same than the Tele's one for
        building Models.
