...
from tele.applets.component.core import ComponentRegistry, MetaComponent, _get_applet_name
...
class ComponentMixin:
    ...
    def setUpComponent(cls):
        with new_rollbacked_env() as env:
            ...
            # build the components of every installed applets
            ...
            # dependencies of the tested applets, not the siblings or
            # children applets
            builder.build_registry(comp_registry, states=("installed",))
            # build the components of the current tested applet
            current_applet = _get_applet_name(cls.__module__)
            env["component.builder"].load_components(current_applet)
            if hasattr(cls, "env"):
...
class TransactionComponentCase(common.TransactionCase, ComponentMixin):
    ...
    It it used like an usual Tele's TransactionCase, but it ensures
    that all the components of the current applet and its dependencies
    are loaded.
...
class ComponentRegistryCase(unittest.TestCase, common.MetaCase("DummyCase", (), {})):
    ...
    If you only want to *use* the components of the tested applet in your tests,
    then consider using:

    * :class:`TransactionComponentCase`

    This test case creates a special
    :class:`tele.applets.component.core.ComponentRegistry` for the purpose of
    the tests. By default, it loads all the components of the dependencies, but
    not the components of the current applet (which you have to handle
    manually). In your tests, you can add more components in 2 manners.

    All the components of an Tele module::
    ...
    def _setup_registry(class_or_instance):
        ...
        # 'component' applet and push them in the component registry
        class_or_instance.comp_registry.load_components("component")
        # build the components of every installed applets already installed
        # but the current applet (when running with pytest/nosetest, we
        # simulate the --test-enable behavior by excluding the current applet
        # which is in 'to install' / 'to upgrade' with --test-enable).
        current_applet = _get_applet_name(class_or_instance.__module__)
        with new_rollbacked_env() as env:
            env["component.builder"].build_registry(
                class_or_instance.comp_registry,
                states=("installed",),
                exclude_applets=[current_applet],
            )
...
class TransactionComponentRegistryCase(common.TransactionCase, ComponentRegistryCase):
    """Adds Tele Transaction in the base Component TestCase.
