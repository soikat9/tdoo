tele.define('dynamic_tele.ViewsCenter', function (require) {
    "use strict";

    var core = require('web.core');
    var BaseCenter = require("dynamic_tele.BaseCenter");
    var basic_fields = require('dynamic_tele.basic_fields');
    // var ActionManager = require('web.ActionManager');
    var ReportCenter = require('dynamic_tele.ReportCenter');
    var AutomationContent = require('dynamic_tele.AutomationContent');
    ...
    var ViewsCenter = BaseCenter.extend({
        ...
        }, action = tele['studio'].state.action, params = {}) {
            tele['studio'].env.bus.trigger("CLEAR-CACHES");
            const {active_id} = tele['studio'].state, ctx = {};
            if (active_id) {
        ...
        renderViewAccess: async function () {
            ...
            await this.loadAction(() => {
                ...
                if (typeEdit != "translation") {
                    domain = [['model_name', '=', tele['studio'].state.model]];
                ...
                self.bindAction();
            }, action, {views: tele['studio'].views[typeEdit]});
        },
        ...
        renderViewViews: async function (wrap) {
            ...
            if (!(this.action && this.action.id == tele['studio'].state.action)) {
                await this.loadAction(() => {
