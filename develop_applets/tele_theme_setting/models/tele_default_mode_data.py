...
class TeleDefaultMode(models.Model):
    '''
    default mode
    '''
    _name = 'tele_theme_setting.default_mode_data'
    _description = 'Tele Theme Setting Mode Template'
    ...
    def get_default_modes(self):
        ...
        """
        records = self.search([])
        return [record.get_mode_data() for record in records]
    ...
    def install_mode(self, config_path, template):
        ...
        tmp_path = get_resource_path(module, file_path)
        if not tmp_path:
            raise exceptions.ValidationError(f'File not found! {config_path}')
        ...
        with open(tmp_path, 'r') as fd:
            ...
            try:
                ...
                raise exceptions.ValidationError('Template {template} must be a valid json file!'.format(template=config_path)) from e
