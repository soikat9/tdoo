...
class TeleThemeVar(models.Model):
    '''
    user theme style setting
    '''
    _name = 'tele_theme_setting.theme_var'
    _order = 'sequence asc'
        ...
        for record in self:
            if record.type == 'image' and record.image:
                record.image_file_url = \
                    '/web/image/tele_theme_setting.theme_var/{var_id}/image'.format(
                        var_id=record.id)
            else:
    ...
    def get_edit_var_action(self):
        ...
        return {
            ...
            "res_model": "tele_theme_setting.theme_var",
            "res_id": self.id,
            "view_mode": "form",
            "target": "new",
            "views": [[self.env.ref(
                'tele_theme_setting.edit_theme_var_form').id, "form"]]
        }
