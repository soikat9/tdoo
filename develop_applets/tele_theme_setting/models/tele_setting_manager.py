...
class TeleThemeSettingManager(models.AbstractModel):
    '''
    user theme style setting
    '''
    _name = 'tele_theme_setting.setting_manager'
    ...
    def _register_hook(self):
        ...
        try:
            ...
            if config.get('debug', False):
                self.env['tele_theme_setting.default_mode_data'].sudo().refresh_default_mode()
                force = True
            self.env["tele_theme_setting.theme_mode"].check_default_mode_data(owner, force)
    ...
    def get_user_setting(self, get_style=True):
        ...
        all_modes = self.env["tele_theme_setting.theme_mode"].search(
            [('owner', '=', owner)])

        cur_style_id = theme_settings["current_theme_style"]
        cur_mode_id = theme_settings["current_theme_mode"]

        theme_mode = self.env["tele_theme_setting.theme_mode"].search(
            [('id', '=', cur_mode_id)])
        theme_style = self.env["tele_theme_setting.theme_style"].search(
            [('id', '=', cur_style_id)])
        ...
        if not cur_mode_id \
            ...
            if all_modes:
                ...
                cur_style_id = False
                cur_mode_name = False
                theme_mode = self.env["tele_theme_setting.theme_mode"]

        if not cur_style_id:
            theme_styles = theme_mode.theme_styles
            theme_style = theme_styles[0] if theme_styles else self.env["tele_theme_setting.theme_style"]
        ...
        rst["window_default_title"] = self.env['ir.config_parameter'].sudo().get_param(
            "tele_theme_setting.window_default_title", "Funenc Tele")
        rst["powered_by"] = self.env['ir.config_parameter'].sudo().get_param(
            "tele_theme_setting.powered_by", "Funenc Tele")
    ...
    def get_font_link(self):
        ...
        if theme_settings.get('font_name', False):
            result = '/tele_theme_setting/static/fonts/{font_name}/fonts.css'.format(font_name=theme_settings.get('font_name'))
        return result
    ...
    def update_cur_style(self, mode_id, style_id):
        ...
        if setting_mode == "system":
            ir_config.set_param("tele_theme_setting.current_theme_mode", mode_id)
            ir_config.set_param("tele_theme_setting.current_theme_style", style_id)
    ...
    def save_style_data(self, style_id, style_data, owner):
        ...
        :return:
        '''
        theme_style = self.env["tele_theme_setting.theme_style"].browse(style_id)
    ...
    def get_all_mode_data(self, owner):
        ...
        get all mode data
        """
        all_modes = self.env["tele_theme_setting.theme_mode"].search([('owner', '=', owner)])
        result = []
