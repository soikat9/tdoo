...
from tele import models, fields, exceptions
import logging, json

_logger = logging.getLogger(__name__)


class TeleStyleSubGroup(models.Model):
    '''
    user theme style group
    '''
    _name = 'tele_theme_setting.style_item_sub_group'
    _order = 'sequence asc'
    ...
    style_items = fields.One2many(
        string="style items",
        comodel_name="tele_theme_setting.style_item",
    ...
    def check_style_items(self, default_mode, style_items):
        ...
        for item_index, style_item in enumerate(style_items):
            if style_item["name"] not in old_style_item_cache:
                self.env["tele_theme_setting.style_item"].create_style_item(
    ...
    def get_sub_group_data(self):
        ...
        :return:
        '''
        return [{"id": record.id, "name": record.name, "style_items": record.style_items.get_style_item_data(), "sequence": record.sequence, "is_default": record.is_default} for record in self]
