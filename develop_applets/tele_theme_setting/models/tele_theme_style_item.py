...
class TeleThemeStyleItem(models.Model):
    '''
    user theme style setting
    '''
    _name = 'tele_theme_setting.style_item'
    _order = 'sequence asc'
    _description = "style item"

    sub_group = fields.Many2one(
        comodel_name="tele_theme_setting.style_item_sub_group", string="sub group")
    ...
    vars = fields.One2many(
        string="vars",
        comodel_name="tele_theme_setting.theme_var",
    ...
    def check_vars(self, default_mode, sub_group_id, default_vars):
        ...
        self.env["tele_theme_setting.theme_var"].create(tmp_vals)
    ...
    def _compute_val(self):    # sourcery skip: low-code-quality
        '''
        compute the val
        :return:
        '''
        ...
        for record in self:
            ...
            for var in record.vars:
                if var.type == 'color':
                    ...
                    try:
                        if background_image and not var.disable_opacity:
            ...
            if (not all_color or not all_ref or not all_same) and not record.vars or all_color and all_ref and all_same:
                record.val = False
            else:
                try:
    ...
    def write(self, vals):
        ...
        :return:
        """
        super(TeleThemeStyleItem, self).write(vals)
    ...
    def get_login_style_data(
        ...
        """
        result = []
        theme_style = self.env["tele_theme_setting.theme_style"].browse(theme_style_id)
