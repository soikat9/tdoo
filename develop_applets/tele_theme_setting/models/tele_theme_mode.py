...
from tele import models, fields, api, exceptions, tools
import json
import re
from tele.tools.config import config
from tele.modules.module import get_resource_path
from .tele_utility import apply_alpha
...
class TeleThemeMode(models.Model):
    ...
    '''
    _name = 'tele_theme_setting.theme_mode'
    ...
    mode_template = fields.Many2one(
        comodel_name="tele_theme_setting.mode_template",
        string="Mode Template")

    template_var_vals = fields.One2many(
        comodel_name="tele_theme_setting.template_var_val",
        inverse_name="theme_mode",
    ...
    def get_mode_preview_data(self, style_id=False):
        ...
        if self.mode_template:
            if style_id:
                theme_style = self.env['tele_theme_setting.theme_style'].browse(
                    style_id)
    ...
    def on_mode_template_changed(self):
        ...
        if self.mode_template:
            ...
            for group_name in scss_vars:
                ...
                template_var_vals.extend((0, 0, {"name": var_item["name"], "group": group_name, "value": var_item["value"], "disable_opacity": var_item.get("disable_opacity", False), "description": var_item["description"]}) for var_item in var_items)
    ...
    def _compile_scss(self, alpha=False):  # sourcery skip: low-code-quality
        """
        ...
        mixins = get_resource_path('tele_theme_base', 'static', 'css')
    ...
    def compute_mode_style_url(self):
        ...
        for record in self:
            record.mode_style_url = \
                '/web/image/tele_theme_setting.tele_theme_setting/{var_id}/icon128x128'.format(
                    var_id=record.id)
    ...
    def check_default_mode_data(self, owner=False, force = False):
        ...
        default_modes = \
            self.env["tele_theme_setting.default_mode_data"].search([])
        ...
        for default_mode in default_modes:
            ...
            if default_mode.name not in olde_mode_cache:
                ...
                # check the templates
                templates = self.env["tele_theme_setting.mode_template"].get_templates(
                )
                ...
                if force or mode.version != mode_data["version"]:
                    ...
                    # update template
                    mode_templates = self.env["tele_theme_setting.mode_template"].get_templates(
                    )
    ...
    def create_mode(self, mode_config, owner=False, is_default=True):
        ...
        '''
        mode_templates = \
                    self.env["tele_theme_setting.mode_template"].get_templates()
                ...
                for sub_group_index, sub_group in enumerate(sub_groups):
                    ...
                    for item_index, style_item in enumerate(style_items):
                        ...
                        for var_index, var_info in enumerate(var_infos):
                            ...
                            if ref_var := var_info.get("ref_var", False):
        ...
        for group_name in template_var_vals:
            ...
            vals.extend((0, 0, {"name": var_item["name"], "group": group_name, "value": var_item["value"], "disable_opacity": var_item.get("disable_opacity", False), "description": var_item["description"]}) for var_item in var_items)

        mode_template = mode_config.get("mode_template", False)
        return self.env["tele_theme_setting.theme_mode"].create([{
    ...
    def check_mode_data(self):
        ...
        default_modes = self.env[
            "tele_theme_setting.default_mode_data"].search([])
        default_mode_cache = {
        ...
        for theme_style in theme_styles:
            ...
            if style_name not in theme_style_cache:
                self.env["tele_theme_setting.theme_style"].create_style(
                    self.id, theme_style)
            else:
    ...
    def get_mode_edit_action(self, style_id):
        ...
        """
        self.ensure_one()
        form_id = self.env.ref("tele_theme_setting.edit_theme_mode_form").id

        var_vals = [(0, 0, {"name": var_val.name, "group": var_val.group, "value": var_val.value, "disable_opacity": var_val.disable_opacity, "description": var_val.description}) for var_val in self.template_var_vals]

        return {
            "type": "ir.actions.act_window",
            "res_model": "tele_theme_setting.theme_mode",
            "views": [[form_id, "form"]],
            "res_id": self.id,
            "view_mode": "form",
            "target": "new",
    ...
    def get_create_new_mode_action(self):
        ...
        """
        form_id = self.env.ref("tele_theme_setting.new_theme_mode_form").id
        return {
            "type": "ir.actions.act_window",
            "res_model": "tele_theme_setting.theme_mode",
            "views": [[form_id, "form"]],
