...
from tele.tools.convert import xml_import, nodeattr2bool
import tele
from tele import api, fields, models, tools
from tele.osv import expression
from .tele_default_menu_icon import svg_icons, font_icons


class TeleMenuExtend(models.Model):
    '''
    ...
    def load_web_menus(self, debug):
        ...
        """
        theme_setting = self.env["tele_theme_setting.setting_manager"].sudo().get_theme_setting()
        menu_icon_policy = theme_setting.get("menu_icon_policy", "svg_icon")

        menus = self.load_menus(debug)
        favorite_cache = {}
        if theme_setting["favorite_mode"]:
            favorite_apps = self.env['tele_theme_setting.favorite_menu'].sudo().search(
