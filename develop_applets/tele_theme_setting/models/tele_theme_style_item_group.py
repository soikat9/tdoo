...
from tele import models, fields, api, exceptions, _, exceptions
import logging, json

_logger = logging.getLogger(__name__)


class TeleStyleGroup(models.Model):
    '''
    user theme style group
    '''
    _name = 'tele_theme_setting.style_item_group'
    _order = 'sequence asc'
    ...
    sub_groups = fields.One2many(
        string="sub groups",
        comodel_name="tele_theme_setting.style_item_sub_group",
        inverse_name="group")
    ...
    def check_sub_group(self, default_mode, sub_groups):
        ...
        for sub_group_index, sub_group in enumerate(sub_groups):
            if sub_group["name"] not in old_sub_group_cache:
                self.env['tele_theme_setting.style_item_sub_group']\
    ...
    def get_add_new_group_action(self, style_id):
        ...
        return {
            ...
            "views": [[self.env.ref('tele_theme_setting.style_item_group_form').id, "form"]]
        }
    ...
    def get_group_data(self):
        ...
        :return:
        """
        return [{"id": record.id, "name": record.name, "is_default": record.is_default, "sequence": record.sequence, "sub_groups": record.sub_groups.get_sub_group_data()} for record in self]
    ...
    def get_add_sub_group_action(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "res_model": "tele_theme_setting.style_item_sub_group",
            "view_mode": 'form',
            "target": "new",
            "context": {
                "default_group": self.id,
            },
            "views": [[self.env.ref('tele_theme_setting.style_item_sub_group_form').id, "form"]]
        }
