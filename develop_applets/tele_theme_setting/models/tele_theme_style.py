...
class TeleThemeStyle(models.Model):
    '''
    Theme style
    '''
    _name = 'tele_theme_setting.theme_style'
    _description = 'tele theme style'
    _order = 'sequence'
    ...
    background_image = fields.Many2one(
        comodel_name="tele_theme_setting.background_image",
    ...
    groups = fields.One2many(
        string="Groups",
        comodel_name="tele_theme_setting.style_item_group",
    ...
    def post_deal_preview_data(self, style_item_infos, font_info, preview = False):
        ...
        mode_prefix = 'body.preview ' if preview else f'body.{self.theme_mode.name} '
        result = [font_info]
        groups = self.groups
        for group in groups:
            for sub_group in group.sub_groups:
        ...
        # append the background image
        if self.background_image:
            if background_image_url := self.background_image.get_image_url():
                result.append(
                    mode_prefix + " {background-image: url('" +
                    background_image_url + "') !important;")
    ...
    def check_groups(self, default_mode, groups):
        ...
        for group_index, name in enumerate(new_group_cache):
            ...
            if name not in old_group_cache:
                self.env['tele_theme_setting.style_item_group']\
    ...
    def create_style(
        ...
        mode = self.env['tele_theme_setting.theme_mode'].browse(model_id)
            ...
            for sub_group_index, sub_group in enumerate(sub_groups):
                ...
                for style_item_index, style_item in enumerate(style_items):
                    ...
                    for var_info in var_infos:
                        if ref_var := var_info.get('ref', False):
        ...
        return self.env['tele_theme_setting.theme_style'].create({
    ...
    def add_new_style(
        ...
        if not owner:
             owner = self.env["tele_theme_setting.setting_manager"].get_current_owner()
    ...
    def import_new_style(self, mode_id, wizard_id):
        ...
        :return:
        """
        wizard = self.env["tele_theme_setting.import_theme_style"].browse(
            wizard_id)
    ...
    def set_background_image(self, wizard_id):
        ...
        """
        self.ensure_one()
        wizard = self.env["tele_theme_setting.background_image_wizard"].browse(
            wizard_id)

        # create background image
        record = self.env["tele_theme_setting.background_image"].create({
            "data": wizard.data,
