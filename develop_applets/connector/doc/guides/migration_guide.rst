...
* The ``ConnectorUnit`` API, which is the core of the composability
  of the Connector. It has been replaced by a standalone applet
  called ``component``. (:ref:`api-component`)

The Connector has been splitted in different applets:

* ``queue_job`` in https://github.com/OCA/queue
* ``component`` in the same repository
* ``component_event`` in the same repository
* ``connector`` uses the 3 applets and the parts specifics to the connectors

This guide will show how to migrate from the old API to the new one.

The previous API will stay until the migration to Tele 11.0.
...
Jobs are now more integrated within the Tele API. They are no longer
standalone functions but are applied on methods of Models.  Another change is
that they have been extracted into their own applet, so obviously the Python
paths change.
...
.. code-block:: python
    ...
    from tele.applets.connector.queue.job import job, related_action
...
.. code-block:: python
    ...
    from tele.applets.queue_job.job import job, related_action
...
* :meth:`tele.applets.queue_job.job.job`
* :meth:`tele.applets.queue_job.job.related_action`
...
.. code-block:: python
    ...
    from tele.applets.connector.session import ConnectorSession
...
* :meth:`tele.applets.queue_job.job.job`
* :meth:`tele.applets.queue_job.models.base.Base.with_delay`

****************
Migrating Events
****************

Events are now handled by the ``component_event`` applet.
...
First you had to create an :class:`~tele.applets.connector.event.Event` instance:
...
.. code-block:: python
    ...
    from tele.applets.connector.event import on_record_create
...
* No more imports are needed for the invocation
  ...
  :meth:`tele.applets.component_event.components.event.CollectedEvents.notify`.
* The name of the event must start with ``'on_'``

Links
-----

* :mod:`tele.applets.component_event.components.event`
...
* The check of the key ``connector_no_export`` in the context can
  be replaced by the decorator :func:`tele.applets.component_event.skip_if`
...
* Backend Models are based on Component's Collections:
  :class:`tele.applets.component.models.collection.Collection`
...
* The inheritance is similar to the Tele's one (without ``_inherits``.
* All components have a Python inheritance on
  :class:`~tele.applets.component.core.AbstractComponent` or
  :class:`~tele.applets.component.core.Component`
* The names are global (as in Tele), so you should prefix them with a namespace
* The name of the classes has no effect
* As in Tele Models, a Component can ``_inherit`` from a list of Components
...
* :ref:`api-component`
* :class:`tele.applets.component.core.AbstractComponent`
...
depending of the Tele version you to:

.. code-block:: python

    from tele.applets.connector.connector import ConnectorEnvironment
...
.. code-block:: python
    ...
    from tele.applets.magentoerpconnect.backend import magento1700, magento1900
...
like Tele Models.  Still, if you need to totally replace a component by
