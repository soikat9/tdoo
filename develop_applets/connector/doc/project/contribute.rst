...
Then, my personal advice is to look at the existing connectors (`Tele
Magento Connector`_, `Tele Prestashop Connector`_). You will also probably
...
- Install Tele, its dependencies, sphinx, sphinx_bootstrap_theme and
  sphinx-intl
- Add `this patch
...
<https://bitbucket.org/anybox/public_buildbot_buildouts/src/tip/tele-connector.cfg>`_::

    $ mkdir buildout && cd buildout
    $ wget https://bitbucket.org/anybox/public_buildbot_buildouts/raw/tip/tele-connector.cfg -O buildout.cfg
    $ wget https://bitbucket.org/anybox/public_buildbot_buildouts/raw/tip/bootstrap.py
    $ python bootstrap.py
    $ bin/buildout
    $ createdb connectordb
    $ bin/start_tele -d connectordb --stop-after-init
