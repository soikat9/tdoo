...
external records into Tele records and conversely.
...
from tele.applets.component.exception import NoComponentError
...
def changed_by(*args):
    ...
    When fields are modified in Tele, we want to export only the
...
def external_to_m2o(field, binding=None):
    ...
    binding in Tele and returns it.
...
def follow_m2o_relations(field):
    ...
    :param field: field "path", using dots for relations as usual in Tele
    """
...
class MapChild(AbstractComponent):
    ...
    * Format the output values to the format expected by Tele or the
    ...
    def format_items(self, items_values):
        ...
        It can be overridden for instance to add the Tele
...
class Mapper(AbstractComponent):
    """A Mapper translates an external record to an Tele record and
    ...
    Submappings
        ...
        same call to :py:meth:`tele.models.BaseModel.create()`.
    ...
    def _build_mapper_component(cls):
        ...
        * "real" Python classes applied on top of existing components (here
        ...
        ::
            ...
            >>> cls.__bases__
            (<class 'tele.applets.connector.tests.test_mapper.ThirdMapper'>,
             <class 'tele.applets.component.core.second.mapper'>,
             <class 'tele.applets.component.core.base'>)
...
class ImportMapper(AbstractComponent):
    ...
    Transform a record from a backend to an Tele record
...
class ExportMapper(AbstractComponent):
    ...
    Transform a record from Tele to a backend record
