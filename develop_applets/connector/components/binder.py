...
class Binder(AbstractComponent):
    ...
    _tele_field = "tele_id"  # override in sub-classes
    _sync_date_field = "sync_date"  # override in sub-classes

    def to_internal(self, external_id, unwrap=False):
        """Give the Tele recordset for an external ID
        ...
        if not bindings:
            if unwrap:
                return self.model.browse()[self._tele_field]
            return self.model.browse()
        bindings.ensure_one()
        if unwrap:
            bindings = bindings[self._tele_field]
        bindings = bindings.with_context(**context)
        return bindings

    def to_external(self, binding, wrap=False):
        """Give the external ID for an Tele binding ID

        :param binding: Tele binding for which we want the external id
    ...
    def bind(self, external_id, binding):
        """Create the link between an external ID and an Tele ID

        :param external_id: external id to bind
        :param binding: Tele record to bind
        :type binding: int
        """
    ...
    def unwrap_binding(self, binding):
        ...
        return binding[self._tele_field]
    ...
    def unwrap_model(self):
        ...
        """
        try:
            column = self.model._fields[self._tele_field]
        except KeyError as err:
            raise ValueError(
                "Cannot unwrap model %s, because it has no %s fields"
                % (self.model._name, self._tele_field)
            ) from err
