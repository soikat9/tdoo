...
Some of the Components have an implementation in the ``Connector`` applet, but
...
:py:class:`~connector.components.binder.Binder`
  The ``binders`` give the external ID or Tele ID from respectively an
  Tele ID or an external ID. A default implementation is available.
...
:py:class:`~connector.components.mapper.Mapper`
  The ``mappers`` transform a external record into an Tele record or
  conversely.
...
from tele.applets.queue_job.exception import RetryableJobError
...
class BaseConnectorComponent(AbstractComponent):
    ...
    def advisory_lock_or_retry(self, lock, retry_seconds=1):
        ...
        :exc:`tele.applets.queue_job.exception.RetryableJobError` so the job
        ...
        See :func:`tele.applets.connector.connector.pg_try_advisory_lock` for
        details.
