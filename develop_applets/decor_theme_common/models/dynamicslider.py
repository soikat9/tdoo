...
class ProductPerPageCountTele(models.Model):
    ...
    def create(self, vals):
        res = super(ProductPerPageCountTele, self).create(vals)
        if vals.get('name') == 0:
            raise Warning(
    ...
    def write(self, vals):
        res = super(ProductPerPageCountTele, self).write(vals)
        if vals.get('name') == 0:
            raise Warning(
                _("Warning! You cannot set 'zero' for product page."))
        if vals.get('default_active_count'):
...
class ProductPerPageTele(models.Model):
    ...
    def write(self, vals):
        res = super(ProductPerPageTele, self).write(vals)
