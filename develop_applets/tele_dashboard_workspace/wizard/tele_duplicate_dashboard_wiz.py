...
class TELECreateDashboardWizard(models.TransientModel):
    ...
    tele_top_menu_id = fields.Many2one('ir.ui.menu', string="Show Under Menu", required=True,
    ...
    def DuplicateDashBoard(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id(
            'tele_dashboard_workspace.tele_duplicate_dashboard_wizard')
    ...
    def tele_duplicate_record(self):
        ...
        dup_dash = self.env['tele_dashboard_workspace.board'].browse(dashboard_id).copy({'tele_dashboard_top_menu_id': self.tele_top_menu_id.id})
        context = {'tele_reload_menu': True, 'tele_menu_id': dup_dash.tele_dashboard_menu_id.id}
        dash_id = self.env['tele_dashboard_workspace.board'].browse(dashboard_id)
        if not dup_dash.tele_dashboard_items_ids:
            for item in dash_id.tele_dashboard_items_ids:
                item.sudo().copy({'tele_dashboard_workspace_board_id': dup_dash.id})
        return {
...
class TELEDeleteDashboardWizard(models.TransientModel):
    ...
    def DeleteDashBoard(self):
        '''this function returns acion id of tele.dashboard.duplicate.wizard'''
        action = self.env['ir.actions.act_window']._for_xml_id(
            'tele_dashboard_workspace.tele_delete_dashboard_wizard')
        action['context'] = {'dashboard_id': self.id}
        return action
    ...
    def tele_delete_record(self):
        '''this function creats record of tele_dashboard_workspace.board and return dashboard action_id'''
        dashboard_id = self._context.get('dashboard_id')
        self.env['tele_dashboard_workspace.board'].browse(dashboard_id).unlink()
