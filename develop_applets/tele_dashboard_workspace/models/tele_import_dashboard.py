...
class TeleDashboardWorkspaceImport(models.TransientModel):
    _name = 'tele_dashboard_workspace.import'
    _description = 'Import Dashboard'

    tele_import_dashboard = fields.Binary(string="Upload Dashboard", attachment=True)
    tele_top_menu_id = fields.Many2one('ir.ui.menu', string="Show Under Menu",
    ...
    def tele_do_action(self):
        for rec in self:
            try:
                ...
                self.env['tele_dashboard_workspace.board'].tele_import_dashboard(tele_result, self.tele_top_menu_id)
                return {
