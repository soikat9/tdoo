tele.define('tele_dashboard_workspace.tele_dashboard', function(require) {
    "use strict";

    var core = require('web.core');
    const { patch } = require('web.utils');
    const { WebClient } = require("@web/webclient/webclient");
//    var export = require('tele_dashboard_workspace.import_button');
    ...
    var TeleGlobalFunction = require('tele_dashboard_workspace.TeleGlobalFunction');

    var TeleQuickEditView = require('tele_dashboard_workspace.quick_edit_view');


    var TeleDashboardWorkspace = AbstractAction.extend({
        ...
        jsLibs: [
            '/tele_dashboard_workspace/static/lib/js/Chart.bundle.min.js',
            '/tele_dashboard_workspace/static/lib/js/gridstack-h5.js',
            '/tele_dashboard_workspace/static/lib/js/chartjs-plugin-datalabels.js',
            '/tele_dashboard_workspace/static/lib/js/pdfmake.min.js',
            '/tele_dashboard_workspace/static/lib/js/vfs_fonts.js',
        ],
        cssLibs: ['/tele_dashboard_workspace/static/lib/css/Chart.css',
            '/tele_dashboard_workspace/static/lib/css/Chart.min.css'
        ],
...
//            css_grid = $('.o_rtl').length>0 ?
            ...
            this.form_template = 'tele_dashboard_workspace_template_view';
...
//                disableOneColumnMode: true,
        ...
        getContext: function() {
            ...
            if(self.state['user_context']['teleDateFilterSelection'] !== undefined && self.teleDateFilterSelection !== 'l_none'){
...
//            self.tele_fetch_data();
        ...
        events: {
            ...
            'click .tele_dashboard_select_item': 'onKsSelectItemClick',
            ...
            'click .tele_duplicate_item': 'onKsDuplicateItemClick',
            'click .tele_move_item': 'onKsMoveItemClick',
            ...
            'show.bs.dropdown .tele_dropdown_container': 'onKsDashboardMenuContainerShow',
            'hide.bs.dropdown .tele_dashboard_item_button_container': 'onKsDashboardMenuContainerHide',
        ...
        teleOnDashboardImportClick: function(ev){
         ev.preventDefault();
            ...
            var dashboard_id = this.tele_dashboard_id;
            this._rpc({
                    model: 'tele_dashboard_workspace.board',
         ...
         ev.preventDefault();
           ...
           var dashboard_id = JSON.stringify(this.tele_dashboard_id);
                ...
                self.getSession().get_file({
                    url: '/tele_dashboard_workspace/export/dashboard_json',
                    data: {
...
//            $('#tele_dashboard_title_input').val(this.tele_dashboard_data.tele_child_boards[this.tele_dashboard_data.tele_selected_board_id][0]);
        ...
        teleRenderChartColorOptions: function(e) {
            ...
            if (!$(e.currentTarget).parent().hasClass('tele_date_filter_selected')) {
                ...
                this._rpc({
                    model: 'tele_dashboard_workspace.item',
                    method: 'write',
...
//                result = self.normalize_dn_data(result);
                ...
                if(self.state['domain_data'] != undefined){
                    self.tele_dashboard_data.tele_dashboard_domain_data=self.state['domain_data']
                    Object.values(self.tele_dashboard_data.tele_dashboard_pre_domain_filter).map((x)=>{
                        if(self.state['domain_data'][x['model']] != undefined){
                            if(self.state['domain_data'][x['model']]['tele_domain_index_data'][0]['label'].indexOf(x['name']) ==-1){
                                self.tele_dashboard_data.tele_dashboard_pre_domain_filter[x['id']].active = false;
                            }
                            else{
                                self.tele_dashboard_data.tele_dashboard_pre_domain_filter[x['id']].active = true;
                            }
                        }
                        else{
                            self.tele_dashboard_data.tele_dashboard_pre_domain_filter[x['id']].active = false;
        ...
        teleRenderDashboard: function() {
            var self = this;
            self.$el.empty();
            self.$el.addClass('tele_dashboard_workspace d-flex flex-column');
            ...
            var $tele_header = $(QWeb.render('teleDashboardWorkspaceHeader', {
...
//                            self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 6, null, 2, 2, items[i].id);
        ...
        _teleMainBodyStyle: function(tele_rgba_background_color, tele_rgba_font_color, tile){
            var background_style = "background-color:" + tele_rgba_background_color + ";color : " + tele_rgba_font_color + ";";
            var tele_rgba_dark_background_color_l2 = this._tele_get_rgba_format(this.tele_get_dark_color(tile.tele_background_color.split(',')[0], tile.tele_background_color.split(',')[1], -10));
            var style_image_body_l2 = "background-color:" + tele_rgba_dark_background_color_l2 + ";";
            return {
...
//            style_main_body = "background-color:" + tele_rgba_background_color + ";color : " + tele_rgba_font_color + ";";
            style_main_body = this._teleMainBodyStyle(tele_rgba_background_color, tele_rgba_font_color, tile).background_style;
...
//                    style_image_body_l2 = "background-color:" + tele_rgba_dark_background_color_l2 + ";";
                    style_image_body_l2 = this._teleMainBodyStyle(tele_rgba_background_color, tele_rgba_font_color, tile).style_image_body_l2;
...
//                    style_image_body_l2 = "background-color:" + tele_rgba_background_color + ";";
                    style_image_body_l2 = this._teleMainBodyStyle(tele_rgba_background_color, tele_rgba_font_color, tile).background_style;
        ...
        tele_container_option: function(chart_title, teleIsDashboardManager, teleIsUser, tele_dashboard_list, chart_id, chart_family, chart_type, teleChartColorOptions){
            var container_data = {
                tele_chart_title: chart_title,
                teleIsDashboardManager: teleIsDashboardManager,
                teleIsUser:teleIsUser,
                tele_dashboard_list: tele_dashboard_list,
                chart_id: chart_id,
                chart_family: chart_family,
                chart_type: chart_type,
                teleChartColorOptions: teleChartColorOptions,
            }
            return container_data;
        },

        tele_set_selected_color_pallet: function($tele_gridstack_container, item){
        ...
        _renderGraph: function(item) {
            ...
            var container_data = this.tele_container_option(chart_title, self.tele_dashboard_data.tele_dashboard_manager, true, self.tele_dashboard_data.tele_dashboard_list, chart_id, chart_family, chart_type, this.teleChartColorOptions);

            var $tele_gridstack_container = $(QWeb.render('tele_gridstack_container', container_data)).addClass('tele_dashboarditem_id');
            self.tele_set_selected_color_pallet($tele_gridstack_container, item);
//            $tele_gridstack_container.find('.tele_li_' + item.tele_chart_item_color).addClass('tele_date_filter_selected');
...
//                self.grid.addWidget($tele_gridstack_container, 0, 0, 13, 4, true, 11, null, 3, null, chart_id);
        ...
        tele_chart_color_pallet: function(gradient, setsCount, palette, item){
            var chartColors = [];
...
////                        chart_data.datasets[i]["tele_data_label_type"] = 'value';
////                    }
...
//                }
//            }
            var chartColors = this.tele_chart_color_pallet(gradient, setsCount, palette, item);
            var datasets = teleMyChart.config.data.datasets;
        ...
        onAddItemTypeClick: function(e) {
            ...
            if (e.currentTarget.dataset.item !== "tele_json") {
                self.do_action({
                    ...
                    view_id: 'tele_dashboard_workspace_list_form_view',
                    views: [
                        [false, 'form']
                    ],
                    target: 'current',
                    context: {
                        ...
                        'form_view_ref': 'tele_dashboard_workspace.item_form_view',
...
//                            framework.unblockUI();
        ...
        _onTeleSaveLayoutClick: function() {
            ...
            if (dashboard_title != false && dashboard_title != 0 && dashboard_title !== self.tele_dashboard_data.name) {
                ...
                var model = 'tele_dashboard_workspace.board';
                ...
                if(this.tele_dashboard_data.tele_selected_board_id && this.tele_dashboard_data.tele_child_boards){
                    ...
                    if (this.tele_dashboard_data.tele_selected_board_id !== 'tele_default'){
                        model = 'tele_dashboard_workspace.child_board';
            ...
            if (dashboard_title ==="") {
                ...
                this._rpc({
                    ...
                    args: ['create', self.tele_dashboard_id, {
                        ...
                        "tele_dashboard_workspace_id": self.tele_dashboard_id,
                        "name": dashboard_title,
                        "tele_active": true,
...
//                    self.$el.find(".grid-stack-item[gs-id=" + item_data.id + "]").append(self.teleRenderToDoDashboardView(item_data));
       ...
       var self = this;
        ...
        _teleSaveCurrentLayout: function() {
            var self = this;
            var grid_config = self.tele_get_current_gridstack_config();
            var model = 'tele_dashboard_workspace.child_board';
...
//            });
            this.tele_kpi_preview_background_style($kpi_preview, tele_rgba_background_color, tele_rgba_font_color, field);
            return $kpi_preview

        },

        tele_kpi_preview_background_style: function($kpi_preview, tele_rgba_background_color, tele_rgba_font_color, field){
            $kpi_preview.find('.tele_dashboarditem_id').css({
                "background-color": tele_rgba_background_color,
                "color": tele_rgba_font_color,
            });
        ...
        onTeleDuplicateItemClick: function(e) {
            ...
            this._rpc({
                ...
                args: [parseInt(tele_item_id), {
                    'tele_dashboard_workspace_board_id': parseInt(dashboard_id)
                }],
            }).then(function(result) {
...
//                  self.teleUpdateDashboardItem()
        ...
        teleChartExportXlsCsv: function(e) {
            ...
            this.getSession().get_file({
                url: '/tele_dashboard_workspace/export/' + e.currentTarget.dataset.format,
                data: {
                    data: JSON.stringify(data)
                },
                complete: framework.unblockUI,
        ...
        teleItemExportJson: function(e) {
            ...
            framework.blockUI();
            this.getSession().get_file({
                url: '/tele_dashboard_workspace/export/item_json',
                data: {
                    data: JSON.stringify(data)
                },
                complete: framework.unblockUI,
    ...
    core.action_registry.add('tele_dashboard_workspace', TeleDashboardWorkspace);
