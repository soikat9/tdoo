...
export class TeleHeader extends Component {
    ...
    setup() {
        ...
        this.overlayService = useService("tele_overlay");
    ...
    on_logo_click() {
        ...
        this.rpc('/web/action/load', { action_id: 'base.action_res_company_form' }).then(function (result) {
            ...
            // result.context = '{"form_view_ref": "tele_theme_enterprise_pro.tele_view_company_pop_form"}'
            result.flags = {
                action_buttons: true,
                headless: true,
            };
    ...
    on_toggler_click() {
        ...
        if (config.device.isMobile) {
            ...
            if ($body.hasClass("navigation-toggle-one")) {
                ...
                localStorage.setItem('tele_menu_mode', 'navigation-toggle-none')
                ...
                if (!$body.hasClass('not_have_child')) {
                    ...
                    localStorage.setItem('tele_menu_mode', 'navigation-toggle-one')
                } else {
                    $body.removeClass("navigation-toggle-two");
                    $body.removeClass("navigation-show");
                    $body.addClass("navigation-toggle-none");
                ...
                localStorage.setItem('tele_menu_mode', 'navigation-toggle-two')
            } else {
                // if nothing
                $body.removeClass("navigation-toggle-two");
                $body.removeClass("navigation-show");
...
TeleHeader.template = "tele_theme_enterprise.header";
TeleHeader.components = { NotUpdatable, ErrorHandler };
