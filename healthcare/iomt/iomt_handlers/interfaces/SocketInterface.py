...
from tele.applets.hw_drivers.main import iomt_devices

_logger = logging.getLogger(__name__)

# Because drivers don't get loaded as normal Python modules but directly in
# load_iomt_handlers called by Manager.run, the log levels that get applied to the tele
...
class SocketInterface(Interface):
    ...
    def create_socket_device(dev, addr):
        ...
        Driver in iomt_devices based on the results of the `supported` call.
        """
    ...
    def replace_socket_device(self, dev, addr):
        ...
        thread defined in iomt_devices that's reading and writing from it. The Driver
        thread can modify both socket_devices and iomt_devices. The Interface thread can
        update iomt_devices based on changes in socket_devices. In order to clean up
        ...
        Interface to do any iomt_devices related cleanup in response.
        After this the new connection can replace the old one.
        """
        driver_thread = iomt_devices.get(addr)
        ...
        # thread terminating and removing the corresponding entry in iomt_devices. In the
        # Interface thread _detected_devices will still contain the old socket device.
        # This means update_iomt_devices won't detect there was a change after
        ...
        # new device would show up in the IoMT app for each key. _detected_devices is a
    ...
    def get_devices(self):
        ...
        # update_iomt_devices in Interface stores the keys() attribute of the value
