...
"""
TODO: EDGE-CASE: 1. TO GREP PROXY PASS OUT OF MULTIPLE Ltele-studioTION BLOCK.
normal vhost copy part 
"""

CLIENT_EMAIL = "account@tele.studio"
...
def create_vhost_https(subdomain, custom_domain, tele_backend, longpolling_backend, docker_vhosts="/opt/app/tele/tele-data/docker_vhosts"):
...
def create_vhost_http(subdomain, custom_domain, tele_backend, longpolling_backend, docker_vhosts="/opt/app/tele/tele-data/docker_vhosts", ssl_flag=False):
    #sed -i 's/.*ssl_certificate\ .*/ ssl_certificate \/this\/is\/test/' ssl.conf
    new_conf = os.path.join(docker_vhosts, custom_domain+".conf")
    _logger.info(locals()) 
...
if __name__ == '__main__':
    ...
    subdomain = "trial_test_4.tele.studio"
    custom_domain = "gc-new.tele.studio"

    main_add(subdomain=subdomain, custom_domain=custom_domain, ssl_flag=True, module_path="/opt/app/tele/tele_applets/tele_saas_kit/")
    
    # EVERTHING UNDERNEATH THIS IS FOR TESTING
    #tele_saas_data = os.getcwd()
    #tele_backend, longpolling_backend = grep_backends_from_conf(tele_saas_data, "test.tele.studio")
    # run_certbot(domain_name="domain.com", client_email="abc@domain.com", webroot_path="/usr/share/nginx/html/", dry_run=True)
    #create_vhost("test.tele.studio", "domain.ml", tele_backend, longpolling_backend, docker_vhosts=tele_saas_data, ssl_flag=True)
