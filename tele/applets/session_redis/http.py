...
from tele.tools.func import lazy_property
...
sentinel_host = os.environ.get('TELE_SESSION_REDIS_SENTINEL_HOST')
sentinel_master_name = os.environ.get(
    'TELE_SESSION_REDIS_SENTINEL_MASTER_NAME'
)
if sentinel_host and not sentinel_master_name:
    raise Exception(
        "TELE_SESSION_REDIS_SENTINEL_MASTER_NAME must be defined "
        "when using session_redis"
    )
sentinel_port = int(os.environ.get('TELE_SESSION_REDIS_SENTINEL_PORT', 26379))
host = os.environ.get('TELE_SESSION_REDIS_HOST', 'localhost')
port = int(os.environ.get('TELE_SESSION_REDIS_PORT', 6379))
prefix = os.environ.get('TELE_SESSION_REDIS_PREFIX')
url = os.environ.get('TELE_SESSION_REDIS_URL')
password = os.environ.get('TELE_SESSION_REDIS_PASSWORD')
expiration = os.environ.get('TELE_SESSION_REDIS_EXPIRATION')
anon_expiration = os.environ.get('TELE_SESSION_REDIS_EXPIRATION_ANONYMOUS')
...
if is_true(os.environ.get('TELE_SESSION_REDIS')):
    if sentinel_host:
    ...
    if is_true(os.environ.get('TELE_SESSION_REDIS_COPY_EXISTING_FS_SESSIONS')):
        copy_fs_sessions(tele.tools.config.session_dir)
    if is_true(os.environ.get('TELE_SESSION_REDIS_PURGE_EXISTING_FS_SESSIONS')):
        # clean the existing sessions on the file system
        purge_fs_sessions(tele.tools.config.session_dir)
