...
class Letsencrypt(models.AbstractModel):
    ...
    def _should_run(self, cert_file, domains):
        ...
        with contextlib.suppress(x509.extensions.ExtensionNotFound):
            names.update(
    ...
    def _respond_challenge_dns(self, domain, token):
        ...
        dns_function = getattr(self, f"_respond_challenge_dns_{provider}")
    ...
    def _respond_challenge_dns_shell(self, domain, token):
        ...
        if script_str := self.env["ir.config_parameter"].get_param("letsencrypt.dns_shell_script"):
            env = os.environ.copy()
            env.update(
    ...
    def _wait_for_record(self, domain, token):
        ...
        while True:
            attempt += 1
            try:
                for record in dns.resolver.query(f"_acme-challenge.{domain}", "TXT"):
