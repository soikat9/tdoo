    ...
    start: function() {
        ...
        return new Promise(function (resolve, reject) {
            self.get_client().then(function (client) {
                ...
                self.on('message:warn', self, function(m) {self.displayNotification({ title: m.title, message: m.message, sticky: m.sticky, type: 'info' }); });

                self.$ifr.appendTo(self.$('.o_content')).css(css).addClass('apps-client');

                resolve();
            }, function() {
                self.displayNotification({ title: _t('Tele Appslets will be available soon'), message: _t('Showing locally available modules'), sticky: true, type: 'info' });
                return self._rpc({
