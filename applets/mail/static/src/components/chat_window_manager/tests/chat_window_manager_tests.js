...
QUnit.test('chat window: basic rendering', async function (assert) {
    ...
    assert.strictEqual(
        ...
        1,
        "should have command to expand thread in Connect"
    );
    assert.strictEqual(
...
QUnit.test('chat window: composer state conservation on toggle discuss', async function (assert) {
    ...
    assert.containsNone(document.body, '.o_ChatWindow', "should not have any chat window after opening Connect");

    await afterNextRender(() => this.messaging.discuss.update({ isOpen: false }));
    assert.strictEqual(
        document.querySelector(`.o_ComposerTextInput_textarea`).value,
        "XDU for the win !",
        "chat window composer should still have the same input after closing Connect"
    );
    assert.containsN(
        document.body,
        '.o_Composer .o_AttachmentCard',
        2,
        "Chat window composer should have 2 attachments after closing Connect"
    );
});
...
QUnit.test('chat window: scroll conservation on toggle discuss', async function (assert) {
    ...
    await this.afterEvent({
        ...
        message: "should wait until channel 20 restored its scroll to 142 after closing Connect",
        predicate: ({ scrollTop, thread }) => {
            return (
                thread &&
                thread.model === 'mail.channel' &&
    ...
    assert.strictEqual(
        ...
        142,
        "chat window scrollTop should still be the same after closing Connect"
    );
});
