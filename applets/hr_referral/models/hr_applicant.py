...
class Applicant(models.Model):
    ...
    leader_id = fields.Many2one('hr.referral.leader', copy=False)
    ...
    def _check_referral_fields_access(self, fields):
        referral_fields = {'name', 'partner_name', 'job_id', 'referral_points_ids', 'earned_points', 'max_points', 'active', 'response_id',
                           'shared_item_infos', 'referral_state', 'user_id', 'leader_id', '__last_update', 'ref_user_id', 'id'}
    ...
    def choose_a_leader(self, leader_id):
        self.ensure_one()
        self_sudo = self.sudo()
        if not self.env.user:
            return
        if self_sudo.ref_user_id == self.env.user and not self_sudo.leader_id:
            # Use sudo, user has normaly not the right to write on applicant
            self_sudo.write({'leader_id': leader_id})
    ...
    def _get_leaders(self, applicant_names):
        return [{
            'id': leader.id,
            'leader': applicant_names.get(leader.id, ''),
            'name': applicant_names.get(leader.id, leader.name),
            'position': leader.position,
            'image': leader.image,
        } for leader in self.env['hr.referral.leader'].search([]) if leader.id in applicant_names]

    def _get_leaders_head(self, applicant_names):
        return [{
            'id': leader.id,
            'leader': applicant_names.get(leader.id, ''),
            'name': leader.name,
            'image': leader.image_head,
        } for leader in self.env['hr.referral.leader'].search([])]
    ...
    def retrieve_referral_welcome_screen(self):
        ...
        applicant_name = {applicant_hired.leader_id.id: applicant_hired.partner_name or applicant_hired.name for applicant_hired in applicants_hired}
        applicant_without_leader = applicants_hired.filtered(lambda r: not r.leader_id)

        # If there are applicant hired without leader and available leaders.
        available_leader_count = self.env['hr.referral.leader'].search_count([])
        if bool(applicant_without_leader) and (len(applicants_hired) - len(applicant_without_leader) < available_leader_count):
            result['choose_new_leader'] = True
            result['new_leader_name'] = applicant_without_leader[0].partner_name or applicant_without_leader[0].name
            result['new_leader_id'] = applicant_without_leader[0].id

            result['leaders'] = self._get_leaders_head(applicant_name)
            return result

        result['leaders'] = self._get_leaders(applicant_name)
