...
var MyReferral = AbstractAction.extend({
    ...
    events: {
        ...
        'click .o_choose_leader_available': '_chooseLeader',
    ...
    willStart: function () {
        ...
        var def = this._rpc({
            ...
            .then(function (res) {
                ...
                self.applicantId = res.new_leader_id;
    ...
    /**
     * Save the new user's leader then restart the view
     *
     * @private
     * @param {MouseEvent} e
     */
    _chooseLeader: function (e) {
        var self = this;
        e.preventDefault();
        var leaderId = parseInt($(e.currentTarget).attr('name'));
        this._rpc({
                model: 'hr.applicant',
                method: 'choose_a_leader',
                args: [[self.applicantId], leaderId],
            })
            .then(function (res) {
                self.do_action({
                    type: 'ir.actions.client',
